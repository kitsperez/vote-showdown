...
export default function PollsCreate() {
    const { data, setData, post, processing, errors, transform } = useForm({
        ...
        access_password: '' as string,
        end_mode: 'duration' as 'duration' | 'deadline',
        duration_seconds: 120 as number,
        deadline_at: '' as string,
        launch: true as boolean,
        options: [
            { label: 'The Flashy Contender', icon: '', image: null },
            { label: 'The Silent Mastermind', icon: '', image: null },
            { label: '', icon: '', image: null },
        ] as { label: string; icon: string; image: File | null }[],
    });

    const patchOption = (i: number, patch: Partial<{ label: string; icon: string; image: File | null }>) =>
    ...
    const removeOption = (i: number) =>
        data.options.length > 2 && setData('options', data.options.filter((_, idx) => idx !== i));
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('polls.store'), { forceFormData: true });
    };
        ...
        <ShowdownLayout title="Poll Creator" subtitle="Build something electrifying!">
            ...
            <form onSubmit={submit} className="flex flex-col gap-6 p-6 md:p-10">
                <div className="max-w-3xl rounded-2xl border-[3px] border-[#1b1b1b] bg-white p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] md:p-8">
                    ...
                    <p className="mt-2 font-mono text-[10px] text-zinc-400 uppercase">Tip: upload an image or type an emoji per option — it shows on the poll.</p>
                    ...
                    <div className="mt-6">
                        ...
                        <input
                            ...
                            placeholder="Leave blank for an open poll"
                            className="w-full rounded-xl border-[3px] border-[#1b1b1b] px-4 py-3 font-bold focus:border-[#e4006c] focus:outline-none"
                        />
                ...
                >
                    <Rocket className="h-5 w-5" /> {data.launch ? 'Create & Launch' : 'Save Draft'}
                </button>
            </form>
        </ShowdownLayout>
    );
